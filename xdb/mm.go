...
func (m *mapper) set(dat []byte, offset int) (uint, bool) {
	...
	return uint(pos * int(SYS_PAGE)), true
}
...
func (m *mapper) add(dat []byte) (uint, bool) {
	// create new document; sanitize and prepend header based
	// on the raw dat supplied. if !ok, print log and return
	doc, ok := document(dat)
	if !ok {
...
func (m *mapper) remove(offset int) bool {
	pages := m.del(offset)
